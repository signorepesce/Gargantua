...
static int string_has_no_nul(const char *json, const JsonToken *tok)
{
    ...
    for (size_t i = start; i < end; i++)
    {
        ...
        if (json[i] == 'u')
        {
            ...
            if (json[i + 1u] == '0' && json[i + 2u] == '0' && json[i + 3u] == '0' && json[i + 4u] == '0')
            {
                return -1;
            }
            i += 4u;
...
{
    if (string_has_no_nul(json, tok) != 0)
    {
        return -1;
    }
...
int body_has_key(str path)
{
    ...
    {
        if (strcmp(path, g_body_fields[i].path) == 0)
        {
            return 1;
        }
    }
...
int body_is_null(str path)
{
    ...
    for (unsigned i = 0u; i < g_body_count; i++)
    {
        if (strcmp(path, g_body_fields[i].path) == 0)
        {
            return g_body_fields[i].is_null;
        }
    }
    return 0;
}
...
static int read_text_field(const FieldInfo *f, void *obj, char *json, const JsonToken *tok)
{
    ...
    if ((tok->type == JSON_PRIM) && ((end - start) == 4) && (memcmp(json + start, "null", 4u) == 0) &&
        ((f->flags & (unsigned)FIELD_NOT_NULL) == 0u))
    {
    ...
    if ((tok->type != JSON_STR) || (unescape_in_place(json, tok) != 0))
    {
        return -1;
    }
...
static int read_int_field(const FieldInfo *f, void *obj, const char *json, const JsonToken *tok)
{
    ...
    if (tok->type != JSON_PRIM)
    {
        return -1;
    }

    long value = 0L;
    if (json_token_long_checked(json, tok, &value) != 0)
    {
        return -1;
    }
...
static int read_scalar(const FieldInfo *f, void *obj, char *json, const JsonToken *toks, int t)
{
    ...
    if (f->kind == FIELD_STR)
    {
        return read_text_field(f, obj, json, &toks[t]);
    }
    if (f->kind == FIELD_INT)
    {
        return read_int_field(f, obj, json, &toks[t]);
    }
    if (f->kind == FIELD_LONG)
    {
        long value = 0L;
        if (json_token_long_checked(json, &toks[t], &value) != 0)
        {
            return -1;
        }
    ...
    {
        double value = 0.0;
        if (json_token_double_checked(json, &toks[t], &value) != 0)
        {
            return -1;
        }
    ...
    {
        int value = 0;
        if (json_token_bool_checked(json, &toks[t], &value) != 0)
        {
            return -1;
        }
...
static int json_read_nested(const TypeInfo *type, void *obj, char *json, const JsonToken *toks, int ntok, int root,
                            unsigned depth, const char *prefix, size_t offset, int partial)
{
    if (depth >= NEST_DEPTH_MAX || toks[root].type != JSON_OBJ)
    {
        return -1;
    }
    int failed = 0;
    ...
    {
        if (toks[k].parent != root || toks[k].type != JSON_STR)
        {
            continue;
        }
        if (string_has_no_nul(json, &toks[k]) != 0)
        {
            field_error_add(prefix, "unknown_field", "field names must not contain NUL");
            failed = -1;
            continue;
        }
        ...
        {
            if (strcmp(key, type->fields[j].name) == 0)
            {
                known = 1;
                break;
            }
        ...
        if (!known)
        {
            ...
            field_error_add(path, "unknown_field", "field is not allowed");
            failed = -1;
        }
        k++;
    }
    for (unsigned i = 0u; i < type->field_count; i++)
    {
        ...
        if (t < 0)
        {
            ...
            {
                field_error_add(path, "required", "field is required");
                failed = -1;
            }
            continue;
        }
        if (g_body_count >= JSON_MAX_TOKENS)
        {
            return -1;
        }
        ...
        if (state->is_null && ((f->flags & FIELD_NOT_NULL) || (f->kind != FIELD_STR && !(f->flags & FIELD_NULLABLE))))
        {
            field_error_add(path, "not_null", "field does not accept null");
            failed = -1;
            continue;
        }
        int rc;
        if (f->kind == FIELD_OBJECT)
        {
            ...
            {
                field_error_add(path, "type", "must be an object");
                failed = -1;
                continue;
            }
            rc = json_read_nested(f->nested, (char *)obj + f->offset, json, toks, ntok, t, depth + 1u, path,
                                  offset + f->offset, partial);
        }
        else
        {
            if (state->is_null)
            {
                memset((char *)obj + f->offset, 0, f->size);
                rc = 0;
            }
            else
            {
                FieldInfo scalar = field_scalar(f);
                rc = read_scalar(&scalar, obj, json, toks, t);
                if (rc == 0)
                {
                    field_set_present(f, obj);
                }
            }
            if (rc != 0)
            {
                field_error_add(path, "type", "invalid value for field type");
            }
            else
            {
                rc = field_validate(f, obj, path);
            }
        }
        if (rc != 0)
        {
...
int json_read_mode(const TypeInfo *type, void *obj, char *json, size_t len, int partial)
{
    ...
    {
        request_fail_code(400, "invalid_json", "expected a JSON object");
        return -1;
    }
    ...
    {
        request_fail_code(400, "invalid_json", "expected a valid JSON object within parser limits");
        return -1;
    }
...
{
    if (!g_body_partial || type != g_body_type || target == NULL || changes == NULL || request_failed())
    {
        return -1;
    }
    for (unsigned i = 0u; i < g_body_count; i++)
    {
        ...
        if (state->field->kind == FIELD_OBJECT)
        {
            continue;
        }
        if (state->offset > type->size || state->field->size > type->size - state->offset)
        {
            return -1;
        }
