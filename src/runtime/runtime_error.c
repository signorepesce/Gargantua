...
int path_param_int(const RequestParams *p, int index, int *ok)
{
    ...
    *ok = 0;
    if ((index < 0) || (index >= p->path_count) || (index >= PATH_PARAM_MAX))
    {
        return 0;
    }
    ...
    if ((errno == ERANGE) || (end == s) || (*end != '\0') || (value < (long)INT_MIN) || (value > (long)INT_MAX))
    {
        return 0;
    }
...
str path_param_text(const RequestParams *p, int index)
{
    ...
    if ((index >= p->path_count) || (index >= PATH_PARAM_MAX))
    {
        return "";
    }
...
void request_fail(int status, str message)
{
    ...
    if (n >= (size_t)FAIL_MESSAGE_LEN)
    {
        n = (size_t)FAIL_MESSAGE_LEN - 1u;
    }
...
{
    if (request_failed() && strcmp(g_error_code, "validation_failed") != 0)
    {
        return;
    }
    request_fail_code(400, "validation_failed", "request validation failed");
    if (g_error_count >= ERROR_FIELDS_MAX)
    {
        g_error_truncated = 1;
        return;
    }
...
int error_json_write(int status, str message, char *out, size_t cap)
{
    ...
    for (unsigned i = 0u; active && i < g_error_count; i++)
    {
        ...
        sink_add(&sink, "{\"field\":");
        sink_quoted(&sink, e->field);
        sink_add(&sink, ",\"code\":");
        sink_quoted(&sink, e->code);
        sink_add(&sink, ",\"message\":");
        sink_quoted(&sink, e->message);
        sink_add(&sink, "}");
    }
