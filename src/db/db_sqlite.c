...
static void db_failure(void)
{
    ...
    else if ((code & 255) == SQLITE_BUSY || (code & 255) == SQLITE_LOCKED || (code & 255) == SQLITE_INTERRUPT ||
             (code & 255) == SQLITE_FULL)
    {
        request_fail_code(503, "database_unavailable", "database temporarily unavailable");
    }
    else
    {
        request_fail_code(500, "database_error", "database operation failed");
    }
}
...
static int sqlite_progress_handler(void *unused)
{
    ...
    return now.tv_sec > t_deadline.tv_sec || (now.tv_sec == t_deadline.tv_sec && now.tv_nsec >= t_deadline.tv_nsec);
}
...
static int sqlite_authorizer(void *unused, int action, const char *a, const char *b, const char *db, const char *source)
{
    ...
    (void)db;
    (void)source;
    if ((action == SQLITE_TRANSACTION || action == SQLITE_SAVEPOINT) && !t_control)
    {
        return SQLITE_DENY;
    }
    ...
    if (action == SQLITE_PRAGMA && a != NULL &&
        (strcasecmp(a, "foreign_keys") == 0 || strcasecmp(a, "writable_schema") == 0 ||
         strcasecmp(a, "journal_mode") == 0))
    {
        return SQLITE_DENY;
    }
    return SQLITE_OK;
...
{
    if (g_db != NULL || busy_ms < 1 || busy_ms > 60000 || query_ms < 1 || query_ms > 60000)
    {
        return -1;
    }
    g_busy_ms = busy_ms;
...
int db_open(str url)
{
    ...
    {
        if (t_depth && !t_failed_level)
        {
            t_failed_level = t_depth;
        }
        db_failure();
    ...
    int enabled = 0;
    if ((sqlite3_prepare_v2(g_db, "PRAGMA foreign_keys", -1, &pragma, NULL) == SQLITE_OK) &&
        (sqlite3_step(pragma) == SQLITE_ROW))
    {
        enabled = sqlite3_column_int(pragma, 0) == 1;
    }
    if (sqlite3_finalize(pragma) != SQLITE_OK)
    {
        enabled = 0;
    }
    if (enabled == 0)
...
static int bind_arguments(sqlite3_stmt *st, const SqlArg *args, int nargs)
{
    ...
    {
        int rc;
        if (args[i].kind == FIELD_KIND_COUNT)
        {
            rc = sqlite3_bind_null(st, i + 1);
        }
        else if (args[i].kind == FIELD_STR)
        {
            rc = (args[i].s != NULL) ? sqlite3_bind_text(st, i + 1, args[i].s, -1, SQLITE_TRANSIENT)
...
static sqlite3_stmt *prepare_statement(str sql, const SqlArg *args, int nargs)
{
    ...
    if (t_depth > 0 && sqlite3_get_autocommit(g_db) != 0)
    {
        if (t_depth && !t_failed_level)
        {
            t_failed_level = t_depth;
...
int db_execute(str sql, const SqlArg *args, int nargs)
{
    ...
    if (nargs > DB_MAX_ARGS || (nargs > 0 && args == NULL))
    {
        return -1;
    }
    if (db_lock() != 0)
    ...
    if ((changed > 0) && (actual != NULL) && (strncasecmp(actual, "INSERT", 6u) == 0) &&
        ((actual[6] == ' ') || (actual[6] == '\t') || (actual[6] == '\r') || (actual[6] == '\n')) &&
        (rowid >= (sqlite3_int64)INT_MIN) && (rowid <= (sqlite3_int64)INT_MAX))
    {
        g_last_id = (int)rowid;
    }
    (void)sqlite3_finalize(st);
...
{
    g_last_id = -1;
    if ((sql == NULL) || (nargs < 0) || (nargs > DB_MAX_ARGS) || ((nargs > 0) && (args == NULL)) ||
        (sqlite3_libversion_number() < 3035000))
    {
        return -1;
    }

    size_t len = 0u;
    while ((len < DB_MAX_SQL) && (sql[len] != '\0'))
    {
        len++;
    }
    if (len >= DB_MAX_SQL)
    ...
    if ((strncasecmp(start, "INSERT", 6u) != 0) ||
        (start[6] != ' ' && start[6] != '\t' && start[6] != '\r' && start[6] != '\n'))
    {
        return -1;
    }
    ...
    int id = -1;
    if ((sqlite3_stmt_readonly(st) == 0) && (sqlite3_column_count(st) == 1) && (sqlite3_step(st) == SQLITE_ROW) &&
        (sqlite3_column_type(st, 0) == SQLITE_INTEGER))
    {
        sqlite3_int64 value = sqlite3_column_int64(st, 0);
        if ((value > 0) && (value <= (sqlite3_int64)INT_MAX) && (sqlite3_step(st) == SQLITE_DONE) &&
            (sqlite3_changes(g_db) == 1))
        {
            id = (int)value;
        }
    }
    ...
    if (id < 0 && t_depth && !t_failed_level)
    {
        t_failed_level = t_depth;
    }
    g_last_id = id;
...
{
    if ((name == NULL) || ((name[0] < 'A' || name[0] > 'Z') && (name[0] < 'a' || name[0] > 'z') && name[0] != '_'))
    {
        return 0;
    }
...
{
    if ((type == NULL) || !is_safe_identifier(type->name) || (type->size == 0u) || (type->field_count == 0u) ||
        (type->field_count > 64u) || (type->fields == NULL))
    {
        return 0;
    }
    unsigned keys = 0u;
    for (unsigned i = 0u; i < type->field_count; i++)
    {
        ...
        size_t need = field_storage_size(f);
        if (!is_safe_identifier(f->name) || (need == 0u) || (f->size != need) || (f->offset > type->size) ||
            (need > (size_t)(type->size - f->offset)) || (f->nested != NULL) ||
            ((f->flags & ~(unsigned)(FIELD_PRIMARY_KEY | FIELD_NOT_NULL | FIELD_UNIQUE | FIELD_NULLABLE)) != 0u))
        {
            return 0;
        }
        if ((f->flags & FIELD_PRIMARY_KEY) != 0u)
        {
            if ((f->kind != FIELD_INT) || (f->flags & FIELD_NULLABLE) || (++keys > 1u))
            {
                return 0;
            }
        }
        ...
        {
            if ((f->kind != FIELD_INT) || !is_safe_identifier(f->references) || !is_safe_identifier(f->reference_key))
            {
                return 0;
            }
        }
        for (unsigned j = 0u; j < i; j++)
        {
            ...
            if ((strcmp(f->name, other->name) == 0) ||
                ((f->offset < (size_t)other->offset + other->size) && (other->offset < (size_t)f->offset + f->size)))
            {
                return 0;
            }
        }
...
static int read_text_column(sqlite3_stmt *st, int c, int sql_type, void *slot)
{
    ...
    int bytes = sqlite3_column_bytes(st, c);
    if (text == NULL || bytes < 0 || memchr(text, 0, (size_t)bytes) != NULL)
    {
        return -1;
    }
...
static int read_integer_column(sqlite3_stmt *st, int c, int sql_type, FieldKind kind, void *slot)
{
    ...
    if (kind == FIELD_INT)
    {
        if ((raw < (sqlite3_int64)INT_MIN) || (raw > (sqlite3_int64)INT_MAX))
        {
            return -1;
        }
        int value = (int)raw;
    ...
    if ((sizeof(long) < sizeof(sqlite3_int64)) && ((raw < (sqlite3_int64)LONG_MIN) || (raw > (sqlite3_int64)LONG_MAX)))
    {
        return -1;
    }
...
static int read_double_column(sqlite3_stmt *st, int c, int sql_type, void *slot)
{
    ...
    if ((sql_type != SQLITE_FLOAT) && (sql_type != SQLITE_INTEGER))
    {
        return -1;
    }
...
static int field_slot_valid(const TypeInfo *type, const FieldInfo *field)
{
    ...
    size_t need = field_storage_size(field);

    return !((need == 0u) || ((size_t)field->size != need) || ((size_t)field->offset > (size_t)type->size) ||
             (need > ((size_t)type->size - (size_t)field->offset)));
}
...
static int read_field(sqlite3_stmt *st, int cols, const FieldInfo *field, void *obj)
{
    ...
    for (int c = 0; (c < cols) && (c < 64); c++)
    {
        ...
        if ((name == NULL) || (strcmp(name, field->name) != 0))
        {
            continue;
        }
        if (found != 0)
        ...
        {
            if ((field->flags & FIELD_NOT_NULL) || (field->kind != FIELD_STR && !(field->flags & FIELD_NULLABLE)))
            {
                return -1;
            }
            continue;
        ...
        char *base = obj;
        FieldInfo scalar = field_scalar(field);
        if (read_column(st, c, &scalar, base + scalar.offset) != 0)
        {
            return -1;
        }
        field_set_present(field, obj);
    }
...
static int row_to_struct(sqlite3_stmt *st, const TypeInfo *type, void *obj)
{
    ...
    if ((type->size == 0u) || (type->field_count == 0u) || (type->field_count > 64u) || (type->fields == NULL))
    {
        return -1;
    }
    ...
    for (unsigned f = 0u; (f < type->field_count) && (f < 64u); f++)
    {
        ...
        if (field_slot_valid(type, field) == 0)
        {
            return -1;
        }
        if (read_field(st, cols, field, obj) != 0)
        {
            return -1;
        }
    }
...
{
    if (!type_valid(type) || (out == NULL) || (sql == NULL) || (nargs < 0) || (nargs > DB_MAX_ARGS) ||
        ((nargs > 0) && (args == NULL)))
    {
        return -1;
    }
    ...
    (void)sqlite3_finalize(st);
    db_unlock();
    if (found < 0 && t_depth && !t_failed_level)
    {
        t_failed_level = t_depth;
    }
    return found;
...
{
    if (!type_valid(type) || (rows == NULL) || (sql == NULL) || (nargs < 0) || (nargs > DB_MAX_ARGS) ||
        ((nargs > 0) && (args == NULL)) || (max <= 0) || (max > SQLITE_MAX_ROWS))
    {
        return -1;
    }
    ...
    if (rc != SQLITE_DONE && count != max)
    {
        db_failure();
    }
    (void)sqlite3_finalize(st);
    db_unlock();
    if (rc != SQLITE_DONE && count != max && t_depth && !t_failed_level)
    {
        t_failed_level = t_depth;
    }
...
int db_begin(void)
{
    ...
    if (t_depth == 0)
    {
        (void)snprintf(sql, sizeof(sql), "BEGIN IMMEDIATE");
    }
    else
    {
        (void)snprintf(sql, sizeof(sql), "SAVEPOINT wrap_sp_%d", t_depth + 1);
    }
    if (run_control_statement(sql) != 0)
    {
        db_unlock();
        return -1;
    }
...
int db_commit(void)
{
    if (t_depth == 0 || t_failed_level || g_db == NULL || sqlite3_get_autocommit(g_db))
    {
        return -1;
    }
    char sql[64];
    if (t_depth == 1)
    {
        (void)snprintf(sql, sizeof(sql), "COMMIT");
    }
    else
    {
        (void)snprintf(sql, sizeof(sql), "RELEASE wrap_sp_%d", t_depth);
    }
    if (run_control_statement(sql) != 0)
    {
        return -1;
...
{
    if (sql == NULL || t_depth == 0 || g_db == NULL)
    {
        return -1;
    }
    statement_deadline();
    ...
    for (int i = 0; i < 256; i++)
    {
        ...
        if (sqlite3_finalize(st) != SQLITE_OK || !done)
        {
            return -1;
        }
    }
...
int db_migration_count(void)
{
    ...
    if (db_execute("CREATE TABLE IF NOT EXISTS _gargantua_migrations "
                   "(version INTEGER PRIMARY KEY, name TEXT NOT NULL, sql TEXT NOT NULL)",
                   SQL_NOARGS) < 0)
    {
        return -1;
    }
...
{
    if (t_depth == 0 || version <= 0 || name == NULL || sql == NULL)
    {
        return -1;
    }
    sqlite3_stmt *st =
        prepare_statement("SELECT name, sql FROM _gargantua_migrations WHERE version = ?", SQL_ARGS(SQL_INT(version)));
    if (st == NULL)
    {
        return -1;
    }
    ...
    if (exists)
    {
        ...
        valid = saved_name != NULL && saved_sql != NULL && (size_t)sqlite3_column_bytes(st, 0) == strlen(name) &&
                (size_t)sqlite3_column_bytes(st, 1) == strlen(sql) && strcmp(saved_name, name) == 0 &&
                strcmp(saved_sql, sql) == 0;
    }
    if (sqlite3_finalize(st) != SQLITE_OK || !valid)
    {
        return -1;
    }
    if (exists)
    ...
    return db_execute("INSERT INTO _gargantua_migrations(version,name,sql) VALUES (?,?,?)",
                      SQL_ARGS(SQL_INT(version), SQL_TEXT(name), SQL_TEXT(sql))) == 1
               ? 1
               : -1;
}
...
{
    if (!type_valid(type) || db_lock() != 0)
    {
        return -1;
    }
    statement_deadline();
    int result = g_db ? sqlite_schema_check(g_db, type) : -1;
    db_unlock();
    return result;
}
