...
static int migration_version(const char *name)
{
    ...
    for (; i < 10u && name[i] >= '0' && name[i] <= '9'; i++)
    {
        ...
        if (version > ((unsigned)INT_MAX - digit) / 10u)
        {
            return -1;
        }
        version = version * 10u + digit;
    }
    if (name[i] != '_' || name[i + 1u] != '_')
    {
        return -1;
    }
    size_t len = strlen(name);
    if (len <= i + 6u || len >= 128u || strcmp(name + len - 4u, ".sql") != 0)
    {
        return -1;
    }
...
static char *read_sql(int directory, const char *name, size_t *total)
{
    ...
    struct stat st;
    char *sql = NULL;
    if (fstat(fd, &st) == 0 && S_ISREG(st.st_mode) && st.st_size > 0 && st.st_size <= (off_t)MIGRATE_FILE_MAX &&
        (size_t)st.st_size <= MIGRATE_TOTAL_MAX - *total)
    {
...
int migrate_run(const char *directory)
{
    ...
    if (dir == NULL)
    {
        (void)fprintf(stderr, "gargantua: configured migrations directory is missing or inaccessible: %s\n", directory);
        return -1;
    }
    ...
    for (int scan = 0; scan < 1024; scan++)
    {
        ...
        if (len < 4u || strcmp(entry->d_name + len - 4u, ".sql") != 0)
        {
            continue;
        }
        int version = migration_version(entry->d_name);
        if (version < 0 || count >= MIGRATE_MAX)
        {
            result = -1;
            break;
        }
        int pos = count;
        while (pos > 0 && files[pos - 1].version > version)
        {
            pos--;
        }
        if ((pos > 0 && files[pos - 1].version == version) || (pos < count && files[pos].version == version))
        {
            result = -1;
            break;
        }
    ...
    if (result == 0)
    {
        result = apply_migrations(files, count);
    }
    for (int i = 0; i < count; i++)
    {
        free(files[i].sql);
    ...
    if (result != 0)
    {
        (void)fprintf(stderr, "gargantua: migration validation or execution failed\n");
    }
    return result;
}
