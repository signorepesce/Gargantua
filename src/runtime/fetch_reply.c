...
int read_reply(Channel *channel, size_t *len, const struct timespec *deadline)
{
    ...
    *len = 0u;
    ...
    {
        if (*len + 1u >= sizeof(g_wire))
        {
            return FETCH_TOO_LARGE;
        }
...
static int decode_chunked(char *body, size_t available, size_t *out_len)
{
    ...
    for (int guard = 0; guard < 4096; guard++)
    {
        ...
        {
            if (size > ((size_t)FETCH_MAX_BODY / 16u))
            {
                return FETCH_TOO_LARGE;
            }
        ...
        {
            return ((write + size) > (size_t)FETCH_MAX_BODY) ? FETCH_TOO_LARGE : FETCH_BAD_REPLY;
        }
...
static int parse_status_line(const char *wire, size_t len, size_t *at)
{
    ...
    if ((len < 14u) || (strncmp(wire, "HTTP/1.", 7u) != 0) || ((wire[7] != '0') && (wire[7] != '1')) ||
        (wire[8] != ' '))
    {
        return -1;
    }
...
static int header_int(const char *line, size_t len, long *out)
{
    ...
    {
        if (value > ((long)FETCH_MAX_BODY + 1L))
        {
            return -1;
        }
...
static int take_header(const char *line, size_t line_len, long *declared, int *chunked)
{
    ...
    {
        if ((*declared >= 0L) || (header_int(colon + 1, value_len, declared) != 0))
        {
            return -1;
        }
        return 0;
    ...
    {
        if ((value_len < 7u) || (strncasecmp(colon + 1 + (value_len - 7u), "chunked", 7u) != 0))
        {
            return -1;
        }
        *chunked = 1;
...
static int take_body(char *body, size_t available, long declared, int chunked, size_t *body_len)
{
    ...
    if (length > (size_t)FETCH_MAX_BODY)
    {
        return FETCH_TOO_LARGE;
    }
...
int parse_reply(char *wire, size_t len, int *status_out, char **body_out, size_t *body_len)
{
    ...
    for (int guard = 0; guard < 128; guard++)
    {
        ...
        if ((line_len > 0u) && (wire[at + line_len - 1u] == '\r'))
        {
            line_len--;
        }
        ...
        if (take_header(wire + at, line_len, &declared, &chunked) != 0)
        {
            return FETCH_BAD_REPLY;
        }
...
int make_deadline(int timeout_ms, struct timespec *deadline)
{
    ...
    int budget = ((timeout_ms > 0) && (timeout_ms <= 60000)) ? timeout_ms : FETCH_TIMEOUT_MS;
                  ...
                  size_t body_len)
{
    ...
    int n = snprintf(out, cap,
                     ...
                     "Content-Length: %zu\r\n"
                     "\r\n",
                     method, target->path, target->host, (content_type != NULL) ? "Content-Type: " : "",
                     (content_type != NULL) ? content_type : "", (content_type != NULL) ? "\r\n" : "", body_len);
...
int resolve_request(str url, Target *target)
{
    ...
    if (url == NULL)
    {
        return FETCH_BAD_URL;
    }
    if (parse_url(url, target) != 0)
    {
        return FETCH_BAD_URL;
    }
    if (host_allowed(target->host) == 0)
    {
