...
static int is_safe_identifier(str name)
{
    ...
    for (size_t i = 0u; i < 64u; i++)
    {
        ...
        if (isalpha(c) == 0 && c != '_' && (i == 0u || isdigit(c) == 0))
        {
            return 0;
        }
    }
...
RowList db_query_page(const TypeInfo *type, str key, str filter, int id, int page, int size)
{
    ...
    if (type == NULL || type->size == 0u || is_safe_identifier(type->name) == 0 || is_safe_identifier(key) == 0 ||
        (filter != NULL && is_safe_identifier(filter) == 0))
    {
    ...
    {
        n = snprintf(sql, sizeof(sql), "SELECT * FROM \"%s\" WHERE \"%s\" = ? ORDER BY \"%s\" LIMIT ? OFFSET ?",
                     type->name, filter, key);
    }
    ...
    int found = filter == NULL ? db_query_many(type, result.items, size + 1, sql,
                                               SQL_ARGS(SQL_INT(size + 1), SQL_INT(page * size)))
                               : db_query_many(type, result.items, size + 1, sql,
                                               SQL_ARGS(SQL_INT(id), SQL_INT(size + 1), SQL_INT(page * size)));
