...
A C11 compiler, and the client library for your database — `libsqlite3` or
`libpq`. OpenSSL is optional and enables https in the outbound client.
...
Two drivers. Pick one in `app/application.properties`:
...
database.url    = postgresql://postgres:secret@db/gargantua
```

`eat` compiles only the driver you named and links only its library, so the
other one costs nothing. The rest of the application is identical — the same
`$table` types, the same generated CRUD.

The generated DDL is portable, with one adjustment made by the postgres driver:
SQLite's `INTEGER PRIMARY KEY` becomes `GENERATED BY DEFAULT AS IDENTITY`, so
`$id` auto-assigns on both.

## Container

The image is only the backend — a static musl build in a `scratch` image. The
...
Opens at http://localhost:8100. `deploy.sh` reads `database.driver` and does
the right thing: with `sqlite` it bind-mounts `./data`; with `postgres` it
starts a `postgres:16-alpine` container on a shared network and runs the
backend against it. The database answers to the host name `db` under both
`deploy.sh` and compose, so the same `database.url` works for either.

The Dockerfile is driver-aware too — it installs `sqlite-static` or
`postgresql-dev` based on the same setting, never both.
...
For the postgres pairing there is also a compose file:
