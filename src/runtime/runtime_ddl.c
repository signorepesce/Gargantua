...
static int ddl_field_valid(const TypeInfo *type, unsigned index, unsigned *keys)
{
    ...
    if ((field->kind == FIELD_OBJECT) || (field->nested != NULL) ||
        ((field->flags & ~(unsigned)(FIELD_PRIMARY_KEY | FIELD_NOT_NULL | FIELD_UNIQUE | FIELD_NULLABLE)) != 0u))
    {
        return 0;
    }
    ...
    {
        if ((field->kind != FIELD_INT) || (field->flags & FIELD_NULLABLE) || (++(*keys) > 1u))
        {
            return 0;
        }
    }
    ...
    {
        if ((field->kind != FIELD_INT) || (is_safe_identifier(field->references) == 0) ||
            (is_safe_identifier(field->reference_key) == 0))
        {
            return 0;
        }
    }
    ...
    for (unsigned j = 0u; j < index; j++)
    {
        ...
        if ((strcmp(field->name, other->name) == 0) || ((field->offset < (size_t)other->offset + other->size) &&
                                                        (other->offset < (size_t)field->offset + field->size)))
        {
            return 0;
        }
    }
...
static int ddl_type_valid(const TypeInfo *type)
{
    ...
    {
        if (ddl_field_valid(type, i, &keys) == 0)
        {
            return 0;
        }
    }
...
static int ddl_write_column(Sink *sink, const FieldInfo *f)
{
    ...
    if ((f->flags & FIELD_NOT_NULL) || (f->kind != FIELD_STR && !(f->flags & FIELD_NULLABLE)))
    {
        sink_add(sink, " NOT NULL");
    }
    if ((f->flags & (unsigned)FIELD_UNIQUE) != 0u)
    {
        sink_add(sink, " UNIQUE");
    }
...
int create_table_sql_write(const TypeInfo *type, char *out, size_t cap)
{
    ...
    out[0] = '\0';
    if ((type_valid(type) == 0) || (ddl_type_valid(type) == 0))
    {
        return -1;
    }
...
int db_create_table(const TypeInfo *type)
{
    ...
    if ((type == NULL) || (create_table_sql_write(type, sql, sizeof(sql)) != 0))
    {
        return -1;
    }
...
static int email_valid(str text)
{
    ...
    if (at == NULL || at == text || (size_t)(at - text) > 64u || strchr(at + 1, '@'))
    {
        return 0;
    }
    if (text[0] == '.' || at[-1] == '.')
...
{
    if (field_is_null(f, obj))
    {
        if (f->flags & FIELD_NOT_NULL)
        {
            field_error_add(path, "not_null", "must not be null");
            return -1;
        }
        return 0;
    }
    ...
    if (f->kind == FIELD_STR)
    {
        ...
        if (r->flags & RULE_SIZE)
        {
            ...
            {
                if (((unsigned char)value[i] & 0xc0u) != 0x80u)
                {
                    n++;
                }
            }
            ...
            {
                (void)snprintf(message, sizeof(message), "length must be between %u and %u Unicode code points",
                               r->size_min, r->size_max);
                field_error_add(path, "size", message);
                return -1;
            }
        }
        if ((r->flags & RULE_EMAIL) && !email_valid(value))
        {
            field_error_add(path, "email", "must be a valid email address");
            return -1;
        }
    }
    else if ((f->kind == FIELD_INT || f->kind == FIELD_LONG) && (r->flags & (RULE_MIN | RULE_MAX)))
    {
        ...
        {
            (void)snprintf(message, sizeof(message), "must be at least %ld", r->min_int);
            field_error_add(path, "min", message);
            return -1;
        }
        if ((r->flags & RULE_MAX) && value > r->max_int)
        {
            (void)snprintf(message, sizeof(message), "must not exceed %ld", r->max_int);
            field_error_add(path, "max", message);
            return -1;
        }
    }
    else if (r->flags & (RULE_MIN | RULE_MAX))
    {
        ...
        if (f->kind == FIELD_INT)
        {
            value = read_int(obj, f->offset);
        }
        else if (f->kind == FIELD_LONG)
        {
            long n;
            memcpy(&n, (const char *)obj + f->offset, sizeof(n));
            value = (long double)n;
        }
        else if (f->kind == FIELD_DOUBLE)
        {
            double n;
            memcpy(&n, (const char *)obj + f->offset, sizeof(n));
            value = n;
        ...
        {
            (void)snprintf(message, sizeof(message), "must be at least %.21Lg", r->min);
            field_error_add(path, "min", message);
            return -1;
        }
        if ((r->flags & RULE_MAX) && value > r->max)
        {
            (void)snprintf(message, sizeof(message), "must not exceed %.21Lg", r->max);
            field_error_add(path, "max", message);
            return -1;
        }
    }
...
{
    if (obj == NULL || bad == NULL || !type_valid(type))
    {
        return -1;
    }
    *bad = NULL;
