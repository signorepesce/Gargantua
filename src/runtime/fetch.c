...
static int deadline_left(const struct timespec *deadline)
{
    ...
    long left = ((deadline->tv_sec - now.tv_sec) * 1000L) + ((deadline->tv_nsec - now.tv_nsec) / 1000000L);
...
int host_allowed(const char *host)
{
    ...
    while (*cursor != '\0')
    {
        ...
        while ((n > 0u) && ((cursor[n - 1u] == ' ') || (cursor[n - 1u] == '\t')))
        {
            n--;
        }

        if ((n == host_len) && (strncasecmp(cursor, host, n) == 0))
        {
            return 1;
        }
...
static int take_authority(const char *rest, const char *end, Target *target)
{
    ...
    size_t host_len = (colon != NULL) ? (size_t)(colon - rest) : (size_t)(end - rest);

    if ((host_len == 0u) || (host_len >= sizeof(target->host)))
    {
        return -1;
    }
    ...
    if ((port_len == 0u) || (port_len >= sizeof(target->port)))
    {
        return -1;
    }
    ...
    {
        if ((target->port[i] < '0') || (target->port[i] > '9'))
        {
            return -1;
        }
    }
...
int parse_url(const char *url, Target *target)
{
    ...
    if ((len == 0u) || (len >= (size_t)FETCH_MAX_URL))
    {
        return -1;
    }
    for (size_t i = 0u; i < len; i++)
    {
        if (url_char_safe((unsigned char)url[i]) == 0)
        {
            return -1;
        }
    }
    ...
    if (memchr(rest, '@', (size_t)(authority_end - rest)) != NULL)
    {
        return -1;
    }

    if (take_authority(rest, authority_end, target) != 0)
    {
        return -1;
    }
...
static int wait_ready(int fd, short events, const struct timespec *deadline)
{
    for (int guard = 0; guard < 1000; guard++)
    {
        ...
        if ((entry.revents & (short)(POLLERR | POLLNVAL)) != 0)
        {
            return -1;
        }
...
static int address_allowed(const struct addrinfo *entry)
{
    ...
    if (strcmp(config_str("fetch.link_local", "deny"), "allow") == 0)
    {
        return 1;
    }
    ...
    {
        const struct sockaddr_in *v4 = (const struct sockaddr_in *)(const void *)entry->ai_addr;
        uint32_t host = ntohl(v4->sin_addr.s_addr);

        if (((host & 0xFFFF0000u) == 0xA9FE0000u) || ((host & 0xFF000000u) == 0x00000000u))
        {
            return 0;
        }
    }
    ...
    {
        const struct sockaddr_in6 *v6 = (const struct sockaddr_in6 *)(const void *)entry->ai_addr;
...
int connect_target(const Target *target, const struct timespec *deadline)
{
    ...
    if (getaddrinfo(target->host, target->port, &hints, &list) != 0)
    {
        return FETCH_NO_HOST;
    }
    ...
    if (fd < 0)
    {
        return (blocked != 0) ? FETCH_BLOCKED : FETCH_NO_CONNECT;
    }
...
int tls_open(Channel *channel, const Target *target, const struct timespec *deadline)
{
    ...
    if ((SSL_set_fd(channel->ssl, channel->fd) != 1) || (SSL_set_tlsext_host_name(channel->ssl, host) != 1) ||
        (SSL_set1_host(channel->ssl, host) != 1))
    {
        return FETCH_TLS_FAILED;
    }
    ...
    for (int guard = 0; guard < 64; guard++)
    {
        ...
        if (wait_ready(channel->fd, events, deadline) != 0)
        {
...
ssize_t channel_write(Channel *channel, const char *data, size_t len, const struct timespec *deadline)
{
    ...
    if (wait_ready(channel->fd, POLLOUT, deadline) != 0)
    {
        return -1;
    }
...
ssize_t channel_read(Channel *channel, char *out, size_t cap, const struct timespec *deadline)
{
    ...
    if (channel->ssl != NULL)
    {
        ...
        int reason = SSL_get_error(channel->ssl, rc);
        if ((reason == SSL_ERROR_ZERO_RETURN) || ((reason != SSL_ERROR_WANT_READ) && (reason != SSL_ERROR_WANT_WRITE)))
        {
            return 0;
        }
    ...
    if (wait_ready(channel->fd, POLLIN, deadline) != 0)
    {
        return -1;
    }
