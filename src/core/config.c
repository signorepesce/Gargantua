...
static int store_entry(const char *key, const char *value)
{
    assert(key != NULL);
    assert(value != NULL);
    if (*key == '\0' || strlen(key) >= CONFIG_KEY_LEN || strlen(value) >= CONFIG_VALUE_LEN)
    {
        return -1;
    }
...
int config_load(const char *path)
{
    ...
    while ((fgets(line, (int)sizeof(line), f) != NULL) && (lineno < CONFIG_MAX_KEYS * 8))
    {
        ...
        *eq = '\0';
        if (store_entry(trim_spaces(text), trim_spaces(eq + 1)) != 0)
        {
            bad++;
        }
    }

    if (ferror(f) || (lineno >= CONFIG_MAX_KEYS * 8 && !feof(f)))
    {
        bad++;
    }
    if (fclose(f) != 0)
...
int config_int(const char *key, int fallback)
{
    ...
    {
        (void)fprintf(stderr,
                      "gargantua: '%s' is not a number in '%s',"
                      " using %d\n",
                      text, key, fallback);
        return fallback;
    }
