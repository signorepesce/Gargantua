...
static const char *content_type_for(const char *path)
{
    ...
    for (size_t i = 0u; i < (sizeof(table) / sizeof(table[0])); i++)
    {
        ...
        if ((len > n) && (strcmp(path + len - n, table[i].suffix) == 0))
        {
            return table[i].type;
        }
    }
...
static int strip_prefix(const char *url_path, const char **rest)
{
    ...
    if (strncmp(url_path, g_prefix, g_prefix_len) != 0)
    {
        return -1;
    }
...
int static_file_read(const char *url_path, char *out, size_t cap, size_t *len, const char **content_type)
{
    ...
    if (static_files_enabled() == 0)
    {
        return -1;
    }

    const char *rest = NULL;
    if (strip_prefix(url_path, &rest) != 0)
    {
        return -1;
    }
    ...
    if ((fstat(fileno(file), &info) != 0) || (!S_ISREG(info.st_mode)) || (info.st_size < 0) ||
        ((size_t)info.st_size > cap) || ((size_t)info.st_size > (size_t)STATIC_MAX_BYTES))
    {
        (void)fclose(file);
        return -1;
    }
