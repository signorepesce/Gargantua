...
static Response take_reply(size_t wire_len)
{
    ...
    if (rc != 0)
    {
        return failure(rc, "invalid response");
    }

    char *copy = request_alloc(length + 1u);
    if (copy == NULL)
    {
        return failure(FETCH_TOO_LARGE, "did not fit in the arena");
    }
...
static Response perform(const char *method, str url, str content_type, str body, int timeout_ms)
{
    ...
    if (rejected != 0)
    {
        return failure(rejected, "the request was rejected");
    }

    if ((content_type != NULL) && (header_value_safe(content_type) == 0))
    {
        return failure(FETCH_BAD_HEADER, "invalid content type");
    }

    size_t body_len = (body != NULL) ? strlen(body) : 0u;
    if (body_len > (size_t)FETCH_MAX_BODY)
    {
        return failure(FETCH_TOO_LARGE, "body too large");
    }

    struct timespec deadline;
    if (make_deadline(timeout_ms, &deadline) != 0)
    {
        return failure(FETCH_TIMEOUT, "clock");
    }
    ...
    if (opened != 0)
    {
        return failure(opened, "connection failed");
    }
    ...
    if ((fetch_send_all(&channel, request, (size_t)n, &deadline) != 0) ||
        ((body_len > 0u) && (fetch_send_all(&channel, body, body_len, &deadline) != 0)))
    {
        channel_close(&channel);
...
{
    return perform("POST", url, (content_type != NULL) ? content_type : "application/json", (body != NULL) ? body : "",
                   timeout_ms);
}

str fetch_json_text(Response response, str key)
{
    if ((response.body == NULL) || (response.length == 0u) || (key == NULL))
    {
        return "";
    }
    ...
    if ((raw_len < 0) || (raw_len >= FETCH_MAX_FIELD))
    {
        return "";
    }
...
int fetch_json_int(Response response, str key, int fallback)
{
    ...
    long value = 0L;
    if (json_token_long_checked(response.body, &tokens[slot], &value) != 0)
    {
        return fallback;
    }
...
int fetch_init(void)
{
    ...
    if ((SSL_CTX_set_min_proto_version(g_tls_context, TLS1_2_VERSION) != 1) ||
        (SSL_CTX_set_default_verify_paths(g_tls_context) != 1))
    {
