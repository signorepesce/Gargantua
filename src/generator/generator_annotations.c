...
static int content_type_valid(const char *text)
{
    ...
    if ((n == 0u) || (n >= GENERATOR_MAX_CONTENT_TYPE))
    {
        return 0;
    }
    ...
    for (size_t i = 0u; i < n; i++)
    {
        ...
        if ((c < 0x20u) || (c > 0x7Eu) || (c == (unsigned char)'"') || (c == (unsigned char)'\\'))
        {
            return 0;
        }
        if (c == (unsigned char)'/')
...
static int parse_interval(const char *text, long *out_ms)
{
    ...
    if ((errno != 0) || (end == text) || (value <= 0) || (value > 86400000L))
    {
        return -1;
    }
    ...
    if (value > (86400000L / scale))
    {
        return -1;
    }

    long total = value * scale;
    if ((total < 100L) || (total > 86400000L))
    {
        return -1;
    }
...
static int store_capacity_valid(const char *text, long *out)
{
    ...
    if ((errno != 0) || (end == text) || (*end != '\0') || (value < 1L) || (value > (long)PAGE_MAX))
    {
        return 0;
    }
...
int parse_store_annotation(Generator *ctx, ParseState *st, const char *line)
{
    ...
    if (store_capacity_valid(size_text, &capacity) == 0)
    {
        generator_error(ctx, st->lineno,
                        "$store requires a count from 1 to %d"
                        " (one page, so _store_list can hold it)",
                        PAGE_MAX);
        return 1;
    }
...
int parse_produces_annotation(Generator *ctx, ParseState *st, const char *line)
{
    ...
    if (strncmp(line, "$produces(", 10u) != 0)
    {
        return 0;
    }

    char value[GENERATOR_MAX_URL];
    if (parse_route_url(ctx, st, line, "$produces", value) != 0)
    {
        return 1;
    }
    ...
    {
        generator_error(ctx, st->lineno, "content type is longer than %d characters",
                        (int)sizeof(st->pending_produces) - 1);
        return 1;
    }
...
int parse_repeat_annotation(Generator *ctx, ParseState *st, const char *line)
{
    ...
    if (strncmp(line, "$repeat(", 8u) != 0)
    {
        return 0;
    }

    char value[GENERATOR_MAX_URL];
    if (parse_route_url(ctx, st, line, "$repeat", value) != 0)
    {
        return 1;
    }
    ...
    {
        generator_error(ctx, st->lineno, "$repeat requires an interval from 100ms to 24h, e.g. \"10s\" (got '%s')",
                        value);
        return 1;
    }
    if (st->task_armed != 0)
...
void parse_task(Generator *ctx, ParseState *st, const char *line)
{
    ...
    if ((extract_return_type(line, return_type, sizeof(return_type)) != 0) ||
        (extract_function_name(line, function_name, sizeof(function_name)) != 0))
    {
    ...
    if (ctx->file_count > 0)
    {
        ctx->files[ctx->file_count - 1].has_task = 1;
    }
}
...
int parse_auth_annotation(Generator *ctx, ParseState *st, const char *line)
{
    ...
    if (strncmp(line, "$role(", 6u) != 0)
    {
        return 0;
    }

    char role[GENERATOR_MAX_URL];
    if (parse_route_url(ctx, st, line, "$role", role) != 0)
    {
        return 1;
    }
    ...
    {
        memcpy(st->pending_role, "admin", 6u);
    }
    else
    {
        memcpy(st->pending_role, "user", 5u);
    }
...
int parse_transactional_annotation(Generator *ctx, ParseState *st, const char *line)
{
    ...
    if (word_equals(line, "$transactional") != 1)
    {
        return 0;
    }
...
int arm_route_annotation(Generator *ctx, ParseState *st, const char *line)
{
    ...
    *paren = '\0';
    ...
    if ((parse_route_url(ctx, st, line, word, url) != 0) || (validate_route_url(ctx, st, url) != 0))
    {
        return 1;
    }
...
void open_type(Generator *ctx, ParseState *st, const char *line)
{
    ...
    if (parse_table_name(ctx, st->lineno, line, name, sizeof(name)) != 0)
    {
        return;
    }
    ...
    if (ctx->file_count > 0)
    {
        ctx->files[ctx->file_count - 1].has_table = 1;
    }
}
