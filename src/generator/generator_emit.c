...
void emit_fields(FILE *out, const ParsedType *type)
{
    ...
    (void)fprintf(out,
                  "_Static_assert(sizeof(%s) <= USHRT_MAX,"
                  " \"$table(%s) is too large\");\n",
                  type->name, type->name);
    ...
    for (int i = 0; (i < type->field_count) && (i < GENERATOR_MAX_FIELDS); i++)
    {
        ...
        (void)fprintf(out,
                      "_Static_assert(offsetof(%s, %s) <= USHRT_MAX,"
                      " \"field offset is too large\");\n"
                      "_Static_assert(sizeof(((%s *)0)->%s) <= USHRT_MAX,"
                      " \"field is too large\");\n",
    ...
    for (int i = 0; (i < type->field_count) && (i < GENERATOR_MAX_FIELDS); i++)
    {
        ...
        (void)fprintf(out,
                      "    { \"%s\", %s,"
                      " (unsigned short)offsetof(%s, %s),"
                      " (unsigned short)sizeof(((%s *)0)->%s), %uu, ",
                      f->name, f->kind, type->name, f->name, type->name, f->name, f->flags);
        ...
        {
            (void)fprintf(out, ", NULL, NULL");
        }
        (void)fprintf(out, ", {%uu, %.21LeL, %.21LeL, %uu, %uu, (%ldL - %dL), (%ldL - %dL)} },\n", f->validation,
                      f->min, f->max, f->size_min, f->size_max, f->min_int == LONG_MIN ? LONG_MIN + 1 : f->min_int,
                      f->min_int == LONG_MIN, f->max_int == LONG_MIN ? LONG_MIN + 1 : f->max_int,
                      f->max_int == LONG_MIN);
    }
...
static void emit_sql_argument(FILE *out, const ParsedField *f, const char *var)
{
    ...
    if (f->flags & 8u)
    {
        (void)fprintf(out, "SQL_OPTION_%s(%s.%s)", f->c_type, var, f->name);
    }
    else
    {
...
void emit_crud_declarations(FILE *out, const ParsedType *t)
{
    ...
    if (find_primary_key(t) == NULL || t->json_only != 0)
    {
        return;
    }
    (void)fprintf(out, "RowList $%s_search(DbQuery wrap_query);\nint $%s_check_schema(void);\n", t->name, t->name);
    for (int i = 0; i < t->field_count; i++)
    {
        const ParsedField *f = &t->fields[i];
        (void)fprintf(out, "RowList $%s_by_%s(%s wrap_value, int wrap_page, int wrap_size);\n", t->name, f->name,
                      f->c_type);
    }
    (void)fprintf(out,
...
static void emit_crud_args(FILE *out, const ParsedType *t, const ParsedField *pk, int include_id)
{
    ...
    if (include_id != 0)
    {
        (void)fprintf(out, "%sSQL_INT(wrap_row.%s)", (first == 1) ? "" : ", ", pk->name);
    }
    (void)fprintf(out, ")");
}
...
static void emit_crud_insert(FILE *out, const ParsedType *t, const ParsedField *pk)
{
    ...
    (void)fprintf(out,
                  ...
                  "    static const char wrap_sql[] = %s",
                  t->name, t->name, pk->name, t->name, bytes != 0 ? "{\n" : "\"");
...
void emit_crud(FILE *out, const ParsedType *t)
{
    ...
    (void)fprintf(out,
                  ...
                  "    {\n        request_raise(request_failed() ? request_fail_status() : 500, request_failed() ? "
                  "request_fail_message() : \"invalid patch context\");\n    }\n"
                  "    return current;\n}\n",
                  t->name, t->name, t->name, t->name, t->name);
    const ParsedField *pk = find_primary_key(t);
    if (pk == NULL || t->json_only != 0)
    {
        return;
    }
    (void)fprintf(out,
                  "\nRowList $%s_search(DbQuery wrap_query)\n{\n"
                  "    return db_search(&%s__type, wrap_query);\n}\n"
                  "\nint $%s_check_schema(void)\n{\n    return db_check_schema(&%s__type);\n}\n",
                  t->name, t->name, t->name, t->name);
    for (int i = 0; i < t->field_count; i++)
    {
        const ParsedField *f = &t->fields[i];
        (void)fprintf(out,
                      "\nRowList $%s_by_%s(%s wrap_value, int wrap_page, int wrap_size)\n{\n"
                      "    return $%s_search($query(.filters = { $filter(%s, \"=\", wrap_value) },\n"
                      "        .count = 1, .page = wrap_page, .size = wrap_size));\n}\n",
                      t->name, f->name, f->c_type, t->name, f->name);
    }
    (void)fprintf(out,
                  "\nRowList $%s_page(int wrap_page, int wrap_size)\n{\n"
    ...
    for (int i = 0; i < t->field_count; i++)
    {
        ...
        (void)fprintf(out,
                      "\nRowList $%s_page_by_%s(int wrap_id, int wrap_page, int wrap_size)\n{\n"
                      "    return $%s_by_%s(wrap_id, wrap_page, wrap_size);\n}\n",
                      t->name, f->name, t->name, f->name);
        (void)fprintf(out, "\n%s $%s_fetch_%s(%s wrap_row)\n{\n", f->references, t->name, f->name, t->name);
        if (f->flags & 8u)
        {
            (void)fprintf(out,
                          "    if (!wrap_row.%s.has_value) { return (%s){0}; }\n"
                          "    return $%s_find(wrap_row.%s.value);\n}\n",
                          f->name, f->references, f->references, f->name);
        }
        else
        {
            (void)fprintf(out, "    return $%s_find(wrap_row.%s);\n}\n", f->references, f->name);
        }
    }
