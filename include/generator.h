...
typedef struct
{
    ...
    int file_index;
    int json_only;
    char source_type[GENERATOR_MAX_NAME];
    int pick_count;
} ParsedType;
...
typedef struct
{
    char signature[GENERATOR_MAX_LINE];
    char name[GENERATOR_MAX_NAME];
    char return_type[GENERATOR_MAX_NAME];
    int file_index;
    int line;
...
typedef struct
{
    ...
    int route_count;
    ParsedFunction functions[GENERATOR_MAX_SERVICES];
    int function_count;
...
void generator_error(Generator *ctx, int line, const char *fmt, ...) __attribute__((format(printf, 3, 4)));
