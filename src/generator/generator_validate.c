...
static int service_type_exists(const Generator *ctx, const char *name, int allow_void)
{
    ...
    if (word_equals(name, "int") == 1 || word_equals(name, "long") == 1 || word_equals(name, "double") == 1 ||
        word_equals(name, "bool") == 1 || word_equals(name, "str") == 1)
    {
        return 1;
    }
    if (allow_void && word_equals(name, "void") == 1)
    {
        return 1;
    }
...
static int route_patterns_overlap(const char *a, const char *b)
{
    ...
    for (int guard = 0; guard < GENERATOR_MAX_ROUTE_SEGMENTS; guard++)
    {
        ...
        if ((dynamic_a == 0) && (dynamic_b == 0) && ((na != nb) || (memcmp(pa, pb, na) != 0)))
        {
            return 0;
        }
        if ((sa == NULL) || (sb == NULL))
        {
            return ((sa == NULL) && (sb == NULL)) ? 1 : 0;
        }
        pa = sa + 1;
        pb = sb + 1;
    }
...
static void validate_nested_field(Generator *ctx, ParsedType *t, int type_slot, const ParsedField *f)
{
    ...
    {
        generator_error(ctx, t->line,
                        "nested field %s.%s needs a declared type inside $json;"
                        " only $not_null is allowed",
                        t->name, f->name);
        return;
    }

    if ((ctx->types[target].file_index == t->file_index) && (target >= type_slot))
    {
        generator_error(ctx, t->line, "declare nested type %s before %s in the same file", f->c_type, t->name);
    }
}
...
static void validate_reference_field(Generator *ctx, const ParsedType *t, ParsedField *f)
{
    ...
    if (target >= 0)
    {
        ...
        {
            if ((ctx->types[target].fields[k].flags & 1u) != 0u)
            {
                key = k;
            }
        }
    ...
    if ((t->json_only != 0) || (strcmp(f->c_type, "int") != 0) || (target < 0) || (key < 0) ||
        (ctx->types[target].json_only != 0))
    {
...
static void validate_type_fields(Generator *ctx)
{
    ...
    for (int i = 0; i < ctx->type_count; i++)
    {
        ...
        generator_select_file(ctx, t->file_index);
        ...
        for (int j = 0; j < t->field_count; j++)
        {
            ...
            {
                validate_nested_field(ctx, t, i, f);
            }
            if (f->references[0] != '\0')
            {
                validate_reference_field(ctx, t, f);
            }
            if ((t->json_only != 0) && ((f->flags & 5u) != 0u))
            {
                generator_error(ctx, t->line, "$id/$unique belong on $table fields");
            }
        }
    }
}
...
static void validate_nesting_depth(Generator *ctx)
{
    ...
    for (int i = 0; i < ctx->type_count; i++)
    {
        generator_select_file(ctx, ctx->types[i].file_index);
        if ((depth[i] == 0) || (depth[i] > 8))
        {
            generator_error(ctx, ctx->types[i].line, "nested type cycle or depth exceeds 8: %s", ctx->types[i].name);
        }
    }
}
...
void generator_select_file(Generator *ctx, int file_index)
{
    assert(ctx != NULL);

    if ((file_index < 0) || (file_index >= ctx->file_count))
    {
        return;
    }
    ...
    size_t n = 0u;
    while ((n < (sizeof(ctx->path) - 1u)) && (src[n] != '\0'))
    {
        n++;
    }
...
static void validate_route_types(Generator *ctx, const ParsedRoute *route)
{
    ...
    if ((service_type_exists(ctx, route->return_type, 1) == 0))
    {
        generator_error(ctx, route->line, "return type '%s' is not a declared $table", route->return_type);
    }

    for (int p = 0; (p < route->param_count) && (p < GENERATOR_MAX_PARAMS); p++)
    {
        const ParsedParam *param = &route->params[p];
        if ((param->is_body != 0) && (type_exists(ctx, param->type) == 0))
        {
            generator_error(ctx, route->line, "body type '%s' is not a declared $table", param->type);
        }
    }
}
...
static void validate_route_clashes(Generator *ctx, int index)
{
    ...
    for (int prior = 0; prior < index; prior++)
    {
        ...
        if (word_equals(route->function_name, other->function_name) == 1)
        {
            generator_error(ctx, route->line, "handler '%s' already has a route annotation", route->function_name);
        }
        if ((word_equals(route->method, other->method) == 1) &&
            (count_url_params(route->url) == count_url_params(other->url)) &&
            (route_patterns_overlap(route->url, other->url) != 0))
        {
            generator_error(ctx, route->line, "route %s %s is ambiguous with %s", route->method, route->url,
                            other->url);
        }
    }
}
...
static void validate_routes(Generator *ctx)
{
    ...
    for (int i = 0; (i < ctx->route_count) && (i < GENERATOR_MAX_ROUTES); i++)
    {
        ...
        if ((strcmp(route->url, "/openapi.json") == 0) || (strcmp(route->url, "/health/live") == 0) ||
            (strcmp(route->url, "/health/ready") == 0))
        {
            generator_error(ctx, route->line, "/openapi.json is reserved for generated OpenAPI");
        }

        generator_select_file(ctx, route->file_index);
...
static void validate_service_clashes(Generator *ctx, int index)
{
    ...
    for (int prior = 0; prior < index; prior++)
    {
        if (word_equals(service->function_name, ctx->services[prior].function_name) == 1)
        {
            generator_error(ctx, service->line, "transactional service '%s' is declared twice", service->function_name);
        }
    }

    for (int route = 0; route < ctx->route_count; route++)
    {
        if (word_equals(service->function_name, ctx->routes[route].function_name) == 1)
        {
            generator_error(ctx, service->line, "'%s' cannot be both a service and an endpoint",
...
static void validate_services(Generator *ctx)
{
    ...
    for (int i = 0; (i < ctx->service_count) && (i < GENERATOR_MAX_SERVICES); i++)
    {
        ...
        generator_select_file(ctx, service->file_index);

        if (service_type_exists(ctx, service->return_type, 1) == 0)
        {
            generator_error(ctx, service->line, "service return type '%s' is not supported", service->return_type);
        }
        for (int p = 0; (p < service->param_count) && (p < GENERATOR_MAX_PARAMS); p++)
        {
            if (service_type_exists(ctx, service->params[p].type, 0) == 0)
            {
                generator_error(ctx, service->line, "service parameter type '%s' is not supported",
                                service->params[p].type);
            }
        }
...
static void validate_stores(Generator *ctx)
{
    ...
    for (int i = 0; (i < ctx->store_count) && (i < GENERATOR_MAX_STORES); i++)
    {
        ...
        generator_select_file(ctx, store->file_index);
        ...
        {
            if (word_equals(type->fields[field].kind, "FIELD_OBJECT"))
            {
                generator_error(ctx, store->line, "$store(%s) requires flat fields; nested field '%s' is unsupported",
                                store->type, type->fields[field].name);
            }
        }
    }
}
...
static void validate_tasks(Generator *ctx)
{
    ...
    for (int i = 0; (i < ctx->task_count) && (i < SCHEDULER_MAX_TASKS); i++)
    {
        ...
        generator_select_file(ctx, task->file_index);

        for (int prior = 0; prior < i; prior++)
        {
            if (word_equals(task->function_name, ctx->tasks[prior].function_name) == 1)
            {
                generator_error(ctx, task->line, "task '%s' is declared twice", task->function_name);
            }
        }
        for (int r = 0; r < ctx->route_count; r++)
        {
            if (word_equals(task->function_name, ctx->routes[r].function_name) == 1)
            {
                generator_error(ctx, task->line, "'%s' cannot be both a task and an endpoint", task->function_name);
            }
        }
    }
}
...
static void validate_file_roles(Generator *ctx)
{
    ...
    for (int i = 0; i < ctx->file_count; i++)
    {
        ...
        if (input->has_table)
        {
            for (int f = 0; f < ctx->function_count; f++)
            {
                if (ctx->functions[f].file_index == i)
                {
                    generator_select_file(ctx, i);
                    generator_error(ctx, ctx->functions[f].line,
                                    "put functions in a separate service or controller file");
                }
            }
        }
