...
static int path_matches(const char *pattern, const char *url, RequestParams *out)
{
    ...
    for (int guard = 0; guard < ROUTE_SEGMENT_MAX; guard++)
    {
        ...
        {
            if (out->path_count >= PATH_PARAM_MAX)
            {
                return 0;
            }
            if (ulen == 0u)
        ...
        else
        {
            ...
            if ((plen > 0u) && (memcmp(pattern + pi, url + ui, plen) != 0))
            {
                return 0;
            }
        }
...
static void params_copy(RequestParams *dst, const RequestParams *src)
{
    ...
    for (int i = 0; (i < src->path_count) && (i < PATH_PARAM_MAX); i++)
    {
        memcpy(dst->path_value[i], src->path_value[i], strlen(src->path_value[i]) + 1u);
    }
...
static const Route *route_find_exact(const char *method, const char *url, RequestParams *out)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if ((r->method[0] != method[0]) || (strcmp(r->method, method) != 0))
        {
            continue;
        }
...
const Route *route_find(const char *method, const char *url, RequestParams *out)
{
    ...
    if ((route == NULL) && (out->invalid == 0) && (strcmp(method, "HEAD") == 0))
    {
        route = route_find_exact("GET", url, out);
    }
    return route;
}
...
{
    static const char *const methods[] = {"GET", "HEAD", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"};
    size_t at = 0u;
    int found = 0;
    int invalid = 0;
    if ((out == NULL) || (cap == 0u))
