...
static int token_copy(char *out, const char *value, const char *key)
{
    ...
    for (size_t i = 0u; i < n; i++)
    {
        ...
        {
            (void)fprintf(stderr,
                          "gargantua: %s has a non-ASCII character"
                          " at position %zu\n",
                          key, i);
            return -1;
        }
...
int auth_init(void)
{
    ...
    {
        (void)fprintf(stderr,
                      "gargantua: security.enabled must be"
                      " true or false, not '%s'\n",
                      enabled);
        return -1;
    }
    ...
    {
        (void)fprintf(stderr, "gargantua: security.user_id and security.admin_id"
                              " must be positive\n");
        return -1;
    }
    if (token_copy(g_token, config_str("security.token", ""), "security.token") != 0 ||
        token_copy(g_admin_token, config_str("security.admin_token", ""), "security.admin_token") != 0)
    {
        return -1;
    }
    if (g_token[0] && strcmp(g_token, g_admin_token) == 0)
    {
        (void)fprintf(stderr, "gargantua: security.token and"
                              " security.admin_token are identical\n");
        return -1;
    }
    if (g_enabled && !g_token[0] && !g_admin_token[0])
    {
        (void)fprintf(stderr, "gargantua: security.enabled=true but no"
                              " token has been set\n");
        return -1;
    }
    return 0;
...
void auth_bind(const void *request)
{
    ...
    if (value == NULL || len <= 7u || strncasecmp(value, "Bearer ", 7u) != 0)
    {
        return;
    }
    value += 7;
...
int auth_gate(int public_route, const char *role, int required)
{
    ...
    if (!required && !g_enabled && (role == NULL || !*role))
    {
        return 0;
    }
    if (!t_user)
...
void auth_require_owner(int id)
{
    if (!t_user)
    {
        request_raise(401, "authentication required");
    }
    else if (id != t_user)
    {
        request_raise(403, "permission denied");
    }
}
...
void auth_require_role(const char *role)
{
    ...
    else if (role == NULL || strcmp(role, t_role) != 0)
    {
        request_raise(403, "permission denied");
    }
}
