...
static void openapi_write_schema(OpenApiWriter *w, const Generator *ctx, const char *type)
{
    ...
    if (scalar != NULL)
    {
        openapi_write_text(w, scalar);
    }
    else
    {
        openapi_write_schema_ref(w, ctx, type);
    }
}

static void openapi_write_type_schema(OpenApiWriter *w, const Generator *ctx, const ParsedType *type, int partial,
                                      unsigned depth);
...
static void openapi_write_validation(OpenApiWriter *w, const ParsedField *field)
{
    ...
    char rule[256];
    int integral = (strcmp(field->kind, "FIELD_INT") == 0) || (strcmp(field->kind, "FIELD_LONG") == 0);

    if ((field->validation & 1u) != 0u)
    {
        if (integral)
        {
            (void)snprintf(rule, sizeof(rule), ",\"minimum\":%ld", field->min_int);
        }
        else
        {
            (void)snprintf(rule, sizeof(rule), ",\"minimum\":%.21Lg", field->min);
        }
    ...
    if ((field->validation & 2u) != 0u)
    {
        if (integral)
        {
            (void)snprintf(rule, sizeof(rule), ",\"maximum\":%ld", field->max_int);
        }
        else
        {
            (void)snprintf(rule, sizeof(rule), ",\"maximum\":%.21Lg", field->max);
        }
        openapi_write_text(w, rule);
    }
    ...
    if ((field->validation & 8u) != 0u)
    {
        openapi_write_text(w, ",\"format\":\"email\"");
    }
    if ((field->flags & 1u) != 0u)
    {
        openapi_write_text(w, ",\"readOnly\":true");
    }
...
static void openapi_write_scalar_field(OpenApiWriter *w, const ParsedField *field)
{
    ...
    if ((field->flags & 8u) && !(field->flags & 2u))
    {
        if (strcmp(field->kind, "FIELD_INT") == 0 || strcmp(field->kind, "FIELD_LONG") == 0)
        {
            scalar = "{\"type\":[\"integer\",\"null\"]}";
        }
        else if (strcmp(field->kind, "FIELD_DOUBLE") == 0)
        {
            scalar = "{\"type\":[\"number\",\"null\"]}";
        }
        else
        {
            scalar = "{\"type\":[\"boolean\",\"null\"]}";
        }
    }
    char prefix[128];
...
static void openapi_write_response(OpenApiWriter *w, const Generator *ctx, const ParsedRoute *route)
{
    ...
    {
        openapi_write_text(w, "{\"type\":\"object\",\"required\":[\"items\",\"page\",\"size\",\"hasMore\"],"
                              "\"properties\":{\"items\":");
    }
    if (route->collection != 0)
    {
        openapi_write_text(w, "{\"type\":\"array\",\"maxItems\":100,\"items\":");
    }
    ...
    if (route->collection == 2)
    {
        openapi_write_text(w, ",\"page\":{\"type\":\"integer\",\"format\":\"int32\",\"minimum\":0},"
...
{
    static const char *const codes[] = {"400", "401", "403", "404", "405", "409",
                                        "413", "415", "429", "500", "503", "default"};
...
static void openapi_write_security(OpenApiWriter *w, const ParsedRoute *route)
{
    ...
    {
        openapi_write_text(w, ",\"security\":[{},{\"BearerAuth\":[]}],"
                              "\"x-security-config\":\"security.enabled\"");
    }
...
static void openapi_write_parameters(OpenApiWriter *w, const Generator *ctx, const ParsedRoute *route)
{
    ...
    for (int i = 0; i < route->param_count; i++)
    {
        ...
        openapi_write_text(w, (param->is_query == 0) ? ",\"required\":true,\"schema\":"
...
static void openapi_write_request_body(OpenApiWriter *w, const Generator *ctx, const ParsedRoute *route)
{
    ...
    for (int i = 0; i < route->param_count; i++)
    {
        if (route->params[i].is_body == 0)
        {
            continue;
        }

        openapi_write_text(w, ",\"requestBody\":{\"required\":true,\"content\":"
...
static void openapi_write_success(OpenApiWriter *w, const Generator *ctx, const ParsedRoute *route)
{
    ...
    if (strcmp(route->return_type, "void") == 0)
    {
        openapi_write_text(w, ",\"responses\":{\"204\":{\"description\":\"No content\"}");
        return;
    }
    int post = (strcmp(route->method, "POST") == 0);

    openapi_write_text(w, post ? ",\"responses\":{\"201\":{\"description\":\"Created\""
                               : ",\"responses\":{\"200\":{\"description\":\"Success\"");

    if (post != 0)
    {
        openapi_write_text(w, ",\"headers\":{\"Location\":{\"description\":"
                              "\"Resource URI, when supplied by the handler\","
                              "\"schema\":{\"type\":\"string\",\"format\":\"uri-reference\"}}}");
    }

    openapi_write_text(w, ",\"content\":{");
    openapi_write_quoted(
        w, ((route->collection == 0) && (strcmp(route->return_type, "str") == 0)) ? "text/plain" : "application/json");
...
{
    if ((ctx->type_count < 0) || (ctx->type_count > GENERATOR_MAX_TYPES) || (ctx->route_count < 0) ||
        (ctx->route_count > GENERATOR_MAX_ROUTES) || (ctx->errors != 0))
    {
        return 0;
    }
    for (int i = 0; i < ctx->type_count; i++)
    {
        if ((ctx->types[i].field_count < 0) || (ctx->types[i].field_count > GENERATOR_MAX_FIELDS))
        {
            return 0;
        }
    }
    for (int i = 0; i < ctx->route_count; i++)
    {
        ...
        if ((route->param_count < 0) || (route->param_count > GENERATOR_MAX_PARAMS) || (route->collection < 0) ||
            (route->collection > 2) || (openapi_method_name(route->method) == NULL))
        {
            return 0;
        }
        if ((strcmp(route->url, "/openapi.json") == 0) && (strcmp(route->method, "GET") == 0))
        {
            return 0;
        }
        for (int j = 0; j < i; j++)
        {
            if ((strcmp(route->function_name, ctx->routes[j].function_name) == 0) ||
                ((strcmp(route->url, ctx->routes[j].url) == 0) && (strcmp(route->method, ctx->routes[j].method) == 0)))
            {
                return 0;
            }
        }
...
{
    openapi_write_text(
        w, "{\"openapi\":\"3.1.1\",\"info\":{\"title\":\"Gargantua API\",\"version\":\"1.0.0\"},\"paths\":{");
    int emitted[GENERATOR_MAX_ROUTES] = {0};
    openapi_write_quoted(w, "/openapi.json");
    openapi_write_text(
        w,
        ":{\"get\":{\"operationId\":\"gg.openapi\",\"responses\":{\"200\":{"
        "\"description\":\"OpenAPI document\",\"content\":{\"application/json\":{\"schema\":{\"type\":\"object\"}}}}");
    openapi_write_error_responses(w);
    openapi_write_text(w, "}}");
    for (int i = 0; i < ctx->route_count; i++)
    {
    ...
    for (int i = 0; i < ctx->route_count; i++)
    {
        ...
        {
            if (strcmp(ctx->routes[i].url, ctx->routes[j].url) != 0)
            {
                continue;
            }
            emitted[j] = 1;
    ...
    openapi_write_text(
        ...
        "\"responses\":{\"GargantuaError\":{\"description\":\"Request failed\","
        "\"content\":{\"application/"
        "json\":{\"schema\":{\"type\":\"object\",\"required\":[\"status\",\"code\",\"error\",\"fields\",\"truncated\"],"
        "\"properties\":{\"status\":{\"type\":\"integer\"},\"code\":{\"type\":\"string\"},\"error\":{\"type\":"
        "\"string\"},\"truncated\":{\"type\":\"boolean\"},\"fields\":{\"type\":\"array\",\"maxItems\":16,\"items\":{"
        "\"type\":\"object\",\"required\":[\"field\",\"code\",\"message\"],\"properties\":{\"field\":{\"type\":"
        "\"string\"},\"code\":{\"type\":\"string\"},\"message\":{\"type\":\"string\"}}}}}}}}}},\"schemas\":{");
    for (int i = 0; i < ctx->type_count; i++)
    {
        if (i != 0)
        {
...
{
    if ((out == NULL) || (ctx == NULL) || (openapi_input_valid(ctx) == 0))
    {
        return -1;
    }
    ...
    if ((fflush(document) != 0) || (fseek(document, 0L, SEEK_SET) != 0))
    {
        failed = 1;
    }
    if (failed == 0)
