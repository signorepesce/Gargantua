...
int client_limit(uint32_t ip, int action)
{
    ...
    for (int i = 0; i < 256; i++)
    {
        if (g_clients[i].used && g_clients[i].ip == ip)
        {
            slot = i;
            break;
        }
        if (!g_clients[i].used || (!g_clients[i].connections && now.tv_sec - g_clients[i].window >= 60))
        {
            empty = i;
        }
    }
...
int queue_pop(uint32_t *ip)
{
    ...
    while ((g_queue.count == 0) && (g_queue.stopping == 0))
    {
...
{
    if ((deadline == NULL) || (seconds <= 0) || (clock_gettime(CLOCK_MONOTONIC, deadline) != 0))
    {
        return -1;
    }
...
static int send_header_body(int sock, const char *header, size_t hlen, const char *body, size_t blen)
{
    ...
    {
        if (set_timeout_until(sock, SO_SNDTIMEO, &deadline) != 0)
        {
            return -1;
        }
...
static int write_cors_headers(char *header, size_t cap, size_t *at, int status)
{
    ...
    if (response_cors)
    {
        const char *vary = response_preflight ? ", Access-Control-Request-Method, Access-Control-Request-Headers" : "";
    ...
    if (response_origin[0] == '\0')
    {
        return 0;
    }

    const char *credentials = (strcmp(config_str("cors.credentials", "false"), "true") == 0)
                                  ? "Access-Control-Allow-Credentials: true\r\n"
                                  : "";

    if (header_append(header, cap, at,
                      "Access-Control-Allow-Origin: %s\r\n"
                      "Access-Control-Expose-Headers: Location\r\n%s",
                      response_origin, credentials) != 0)
    {
        return -1;
    }

    if ((response_preflight == 0) || (status != 204))
    {
        return 0;
    }

    return header_append(header, cap, at, "Access-Control-Allow-Methods: %s\r\n%s%s%s", response_allow,
                         response_requested_headers[0] ? "Access-Control-Allow-Headers: " : "",
                         response_requested_headers, response_requested_headers[0] ? "\r\n" : "");
}
...
int send_bytes(int sock, int status, const char *content_type, const char *body, size_t blen, int keep_alive)
{
    ...
    if (header_append(header, cap, &at,
                      ...
                      "Connection: %s\r\n",
                      status, status_reason(status), content_type, (keep_alive == 1) ? "keep-alive" : "close") != 0)
    {
        return -1;
    }
    ...
    int n = response_write(header + at, cap - at, status);
    if (n < 0)
    {
        return -1;
    }
    ...
    if (write_cors_headers(header, cap, &at, status) != 0)
    {
        return -1;
    }
    ...
    if (send_header_body(sock, header, at, body, response_head ? 0u : blen) != 0)
    {
        return -1;
    }
