...
{
    if ((c >= (unsigned char)'0' && c <= (unsigned char)'9') || (c >= (unsigned char)'A' && c <= (unsigned char)'Z') ||
...
static int parse_request_line(const char **p, const char *end, HttpRequest *req)
{
    ...
    if (read_token(p, end, req->method, sizeof(req->method)) != 0 || *p >= end || **p != ' ')
    {
        return -1;
    }
    (*p)++;
    ...
    if (n == 0u || *p >= end || **p != ' ' || (start[0] != '/' && !(n == 1u && start[0] == '*')))
    {
        return -1;
    }
    ...
    if ((size_t)(end - *p) < 8u || memcmp(*p, "HTTP/1.", 7u) != 0 || ((*p)[7] != '0' && (*p)[7] != '1'))
    {
        return -1;
    }
...
static int parse_content_length(const char *value, size_t len, long *out)
{
    ...
    for (size_t i = 0u; i < len; i++)
    {
        ...
        if (value_num > ((unsigned long)LONG_MAX - digit) / 10ul)
        {
            return -1;
        }
...
static int parse_header_name(const char **p, const char *end, size_t *name_len)
{
    ...
    *name_len = 0u;
    while ((*p < end) && (**p != ':'))
    {
        if (is_token_char((unsigned char)**p) == 0)
        {
            return -1;
        }
        (*name_len)++;
...
static int parse_header_value(const char **p, const char *end, const char *value, size_t *value_len)
{
    ...
    *value_len = 0u;
    ...
    {
        unsigned char c = (unsigned char)**p;
        if (((c < 0x20u) && (c != (unsigned char)'\t')) || (c == 0x7fu))
        {
            return -1;
        }
        (*value_len)++;
...
static int count_known_header(const HttpHeader *h, HeaderCounts *seen, HttpRequest *req)
{
    ...
    {
        seen->host++;
        return ((seen->host > 1) || (host_value_valid(h->value, h->value_len) == 0)) ? -1 : 0;
    }
    ...
    if (header_name_is(h, "Authorization") != 0)
    {
        return (++seen->authorization > 1) ? -1 : 0;
    }
    if (header_name_is(h, "Content-Type") != 0)
    {
        return (++seen->content_type > 1) ? -1 : 0;
    }
    if (header_name_is(h, "Expect") != 0)
    {
        return (++seen->expect > 1) ? -1 : 0;
    }
...
static int parse_headers(const char **p, const char *end, HttpRequest *req)
{
    ...
    while (*p < end)
    {
        ...
        {
            *p += 2;
            if ((req->minor_version == 1) && (seen.host != 1))
            {
                return -1;
            }
            return ((seen.transfer_encoding == 0) && (seen.content_length <= 1)) ? 0 : -1;
        }
        ...
        const char *name = *p;
        size_t name_len = 0u;
        if (parse_header_name(p, end, &name_len) != 0)
        {
            return -1;
        }
        ...
        const char *value = *p;
        size_t value_len = 0u;
        if (parse_header_value(p, end, value, &value_len) != 0)
        {
            return -1;
        }
        if (expect_crlf(p, end) != 0)
        {
            return -1;
        }
        ...
        if (count_known_header(h, &seen, req) != 0)
        {
            return -1;
        }
    }
...
HttpParseResult http_parse_request(char *buf, size_t len, HttpRequest *req)
{
    ...
    {
        return (len > HTTP_MAX_HEADER_BYTES) ? HTTP_PARSE_ERROR : HTTP_PARSE_NEED_MORE;
    }
    if (headers > HTTP_MAX_HEADER_BYTES)
    {
        return HTTP_PARSE_ERROR;
    }
    ...
    if (parse_request_line(&p, end, req) != 0 || parse_headers(&p, end, req) != 0)
    {
        return HTTP_PARSE_ERROR;
    }
    ...
    req->body = p;
    if (req->content_length >= 0 && (unsigned long long)req->content_length > (unsigned long long)SIZE_MAX)
    {
        return HTTP_PARSE_ERROR;
    }
    req->body_len = req->content_length >= 0 ? (size_t)req->content_length : 0u;
    if (req->body_len > (size_t)(end - p))
    {
        return HTTP_PARSE_NEED_MORE;
    }
