...
static int token_string(const char *js, size_t len, size_t *at, JsonToken *toks, int max, int *count, int parent)
{
    ...
    while (i < len)
    {
        ...
        if (c == '\\')
        {
            ...
            unsigned first = 0u;
            if (read_hex4(js, len, i + 1u, &first) != 0)
            {
                return -1;
            }
            i += 5u;
            ...
            {
                unsigned second = 0u;
                if (((len - i) < 6u) || (js[i] != '\\') || (js[i + 1u] != 'u') ||
                    (read_hex4(js, len, i + 2u, &second) != 0) || (second < 0xDC00u) || (second > 0xDFFFu))
                {
                    return -1;
                }
                i += 6u;
...
{
    size_t start = *at;
    if ((js[start] == 't') && ((len - start) >= 4u) && (memcmp(js + start, "true", 4u) == 0))
    {
        *at += 4u;
    }
    else if ((js[start] == 'f') && ((len - start) >= 5u) && (memcmp(js + start, "false", 5u) == 0))
    {
        *at += 5u;
    }
    else if ((js[start] == 'n') && ((len - start) >= 4u) && (memcmp(js + start, "null", 4u) == 0))
    {
        *at += 4u;
    }
    else if (scan_number(js, len, at) != 0)
    {
        return -1;
    }
    if (is_delimiter(js, len, *at) == 0)
    {
        return -1;
    }
...
static int scan_value(const char *js, size_t len, size_t *at, JsonToken *toks, int max, int *count, int parent,
                      Frame *stack, int *depth)
{
    if (*at >= len)
    {
        return -1;
    ...
    if (c == '"')
    {
        return (token_string(js, len, at, toks, max, count, parent) < 0) ? -1 : 0;
    }
    if ((c == '{') || (c == '['))
    {
        ...
        stack[*depth].state = (type == JSON_OBJ) ? OBJ_KEY_OR_END : ARR_VALUE_OR_END;
...
static int duplicate_key(const char *js, const JsonToken *toks, int parent, int key)
{
    ...
    for (int prior = parent + 1; prior < key; prior++)
    {
        ...
        if (((direct & 1) == 0) && (toks[prior].type == JSON_STR))
        {
            ...
            if (strcmp(decoded_prior, decoded_key) == 0)
            {
                return 1;
            }
        }
...
static int scan_object_key(const char *js, size_t len, size_t *i, JsonToken *toks, int max, int *count, Frame *f,
                           int *depth, int *done)
{
    assert(js != NULL);
    assert(f != NULL);
    ...
    if ((key < 0) || (f->members >= JSON_MAX_OBJECT_KEYS))
    {
        return -1;
    }

    int key_len = toks[key].end - toks[key].start;
    if ((key_len < 0) || (key_len >= JSON_MAX_KEY_BYTES))
    {
        return -1;
    }
    if (duplicate_key(js, toks, f->token, key) != 0)
    {
        return -1;
    }
...
static int scan_array_element(const char *js, size_t len, size_t *i, JsonToken *toks, int max, int *count, Frame *f,
                              Frame *stack, int *depth, int *done)
{
    assert(js != NULL);
    assert(f != NULL);
...
static int scan_separator(const char *js, size_t *i, Frame *f, JsonToken *toks, int *depth, int *done, char closer,
                          FrameState next)
{
    assert(js != NULL);
    assert(f != NULL);
...
static int scan_frame(const char *js, size_t len, size_t *i, JsonToken *toks, int max, int *count, Frame *stack,
                      int *depth, int *done)
{
    assert(js != NULL);
    assert(stack != NULL);
...
{
    if ((js == NULL) || (toks == NULL) || (max <= 0) || (len == 0u) || (len > (size_t)INT_MAX))
    {
        return -1;
    }
    ...
    while (i < len)
    {
        while ((i < len) && (is_space((unsigned char)js[i]) == 1))
        {
            i++;
        }
        if (i >= len)
        ...
        if (depth == 0)
        {
            ...
            seen = 1;
            if (scan_value(js, len, &i, toks, max, &count, -1, stack, &depth) != 0)
            {
                return -1;
            }
            if (depth == 0)
        ...
        if (scan_frame(js, len, &i, toks, max, &count, stack, &depth, &done) != 0)
        {
            return -1;
        }
    }
...
void json_copy_str(const char *js, const JsonToken *t, char *out, size_t outsz)
{
    ...
    while ((i < end) && ((o + 1u) < outsz))
    {
        ...
        else if (c == 'u')
        {
            ...
            {
                unsigned second = 0u;
                if (read_hex4(js, end, i + 2u, &second) != 0)
                {
                    break;
                }
...
int json_object_get(const char *js, const JsonToken *toks, int ntok, int obj, const char *key)
{
    ...
    assert(toks != NULL);
    assert(key != NULL);
    if ((obj < 0) || (obj >= ntok) || (toks[obj].type != JSON_OBJ))
    {
        return -1;
    }

    size_t klen = strlen(key);
    if (klen >= JSON_MAX_KEY_BYTES)
    {
        return -1;
    }
    int direct = 0;
    for (int i = obj + 1; (i < ntok) && (toks[i].start < toks[obj].end); i++)
    {
        ...
        if (((direct & 1) == 0) && (toks[i].type == JSON_STR))
        {
            ...
            if ((raw_len >= 0) && (raw_len < JSON_MAX_KEY_BYTES))
            {
                ...
                if ((strlen(decoded) == klen) && (memcmp(decoded, key, klen) == 0))
                {
...
int json_path_get(const char *js, const JsonToken *toks, int ntok, int root, const char *path)
{
    ...
    for (int guard = 0; (*p != '\0') && (cur >= 0) && (guard < JSON_MAX_DEPTH); guard++)
    {
        ...
        else if (toks[cur].type == JSON_ARR)
        {
            ...
            if ((errno != 0) || (endp == seg) || (*endp != '\0') || (want < 0) || (want > INT_MAX))
            {
                return -1;
            }
            int idx = 0, found = -1;
...
{
    if ((js == NULL) || (t == NULL) || (buf == NULL) || (cap < 2u) || (t->type != JSON_PRIM) || (t->start < 0) ||
        (t->end <= t->start))
    {
        return -1;
    }
...
int json_token_long_checked(const char *js, const JsonToken *t, long *out)
{
    ...
    if ((out == NULL) || (token_text(js, t, buf, sizeof(buf)) != 0))
    {
        return -1;
    }
    errno = 0;
...
int json_token_double_checked(const char *js, const JsonToken *t, double *out)
{
    ...
    if ((errno == ERANGE) || (end == buf) || (*end != '\0') || !isfinite(value))
    {
        return -1;
    }
    *out = value;
